...
public class DataReaderListenerImpl extends DDS._DataReaderListenerLocalBase {
    ...
    private static final int N_EXPECTED = 5000;
    ...
    public synchronized void on_data_available(DDS.DataReader reader) {
        ...
        if (status == RETCODE_OK.value) {
            ...
            if (sih.value.valid_data) {
                ...
                String type= "From Subscriber reader"; //PUB SUB DCPSINFO
                String topic = custom_topic; // 
                ...
                System.out.println(prefix + "DDS-NAS Message: subject    = " + mh.value.subject);
                System.out.println("         subject_id = "
                                   + mh.value.subject_id);
                System.out.println("         Message from       = " + mh.value.from);
                System.out.println("         Message count      = " + mh.value.count);
                System.out.println("         Message Command       = " + mh.value.text);
                //System.out.println("MY TEST ---");
                //date= new java.util.Date();
                msgBuffer = msgBuffer +   new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(Calendar.getInstance().getTime()) + " | " + System.currentTimeMillis() + " | "+System.nanoTime() +" | DDS-NAS-SUB " + " | 1 | " + type +" | "+  topic +" | "+ mh.value.count +" | "+ mh.value.subject +" | "+
                		mh.value.text + "\n";
                //bufferdMsg = bufferdMsg +
                //System.out.println("SampleInfo.sample_rank = " + sih.value.sample_rank);

                /*if (invalid_count == true) {
                    System.out.println("ERROR: Invalid message.count (" + mh.value.count + ")");
                }
                if (!mh.value.from.equals("OpenDDS-Java-pub") && !mh.value.from.equals("OpenDDS-Java")) {
              ...
              try{
                    ...
                    String data = msgBuffer;
                    File file =new File("/tmp/log/DDS/subDDS.log");
